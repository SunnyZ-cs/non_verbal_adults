...
def load_test_orders(json_path):
    """
    Loads test orders, contexts, and child ages from the Lookit response JSON.
    Returns a dictionary: response_uuid -> {'test_order': ..., 'direction': ..., 'context': ..., 'age_days': ..., 'age_years': ...}
    """
    metadata = {}
    try:
...
def main():
    ...
    summary_df.to_csv("icatcher_summary_results.csv", index=False)
    print("\nSaved summary results to icatcher_summary_results.csv")
