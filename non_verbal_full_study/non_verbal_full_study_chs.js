// ════════════════════════════════════════════════════════════════════
//  NON-VERBAL ADULTS PILOT COMBINED STUDY — CHS jsPsych version
//
...
// Random Context Assignment (chains vs single cause)
...
const distal_anim = BASE + `${context}_` + (is_forward ? 'distal_test_final.gif' : 'reverse_distal_test_final.gif');
const distal_freeze = BASE + `${context}_` + (is_forward ? 'distal_test_final_freeze.png' : 'reverse_distal_test_final_freeze.png');

const proximal_anim = BASE + `${context}_` + (is_forward ? 'proximal_test_final.gif' : 'reverse_proximal_test_final.gif');
const proximal_freeze = BASE + `${context}_` + (is_forward ? 'proximal_test_final_freeze.png' : 'reverse_proximal_test_final_freeze.png');
...
// Assigned GIF name based on condition and order
const assigned_combo = is_forward ? 
    (test_order[0].name === 'distal' ? `${context}_distal_test_final.gif` : `${context}_proximal_test_final.gif`) :
    (test_order[0].name === 'distal' ? `${context}_reverse_distal_test_final.gif` : `${context}_reverse_proximal_test_final.gif`);
...
function buildTestTimeline(testObj) {
    ...
    // 0.5. Pulsing Bullseye to refixate gaze (3 seconds)
    trials.push({
        type: jsPsychHtmlKeyboardResponse,
        stimulus: `
...
const debrief_page = {
    ...
    stimulus: `
        <div class="instructions-box" style="max-width: 800px; margin: 40px auto; text-align: left; line-height: 1.7; font-family: Arial, sans-serif;">
            ...
            <p style="margin-bottom: 1.5em; font-size: 1.05em; color: #444;">With this design, we use a non-verbal eye-tracking method. By measuring where children look on the screen while watching shapes interact, we are testing whether younger children can represent these causal structures implicitly, even when they struggle to express these relationships verbally.</p>
            
            <p style="margin-bottom: 1.5em; font-size: 1.05em; color: #444;"><strong>How this study answers the question:</strong> We record where your child looks during the final moments of the videos. Depending on whether your child was randomly assigned to the causal chain or the single cause scenario, we measure their looking patterns at the star-and-wand authority figure and the shapes. Comparing looking patterns across the two conditions allows us to see how deeply ingrained and automatic our causal reasoning and responsibility attribution are.</p>
