...
DEST="$HOME/Desktop/mediapipe_results_${RUN_NAME}"
mkdir -p "$DEST"
...
# confirmed live on real data. Resolving it once via plain ssh (which
# always uses a real remote shell for command execution, regardless of
# rsync's behavior) sidesteps this for good.
...
rsync -avP -e "ssh ${SSH_OPTS[*]}" "${LOGIN_HOST}:${REMOTE_DIR}/mediapipe_summary_combined.csv" "$DEST/"
...
echo ""
echo "Saved to $DEST"
echo "(Skipping the raw mediapipe_output/ per-frame annotation files - those"
echo " can add up; pull them too with an extra rsync if you need them:"
echo "   rsync -avP ${LOGIN_HOST}:${REMOTE_DIR}/mediapipe_output/ $DEST/mediapipe_output/ )"
