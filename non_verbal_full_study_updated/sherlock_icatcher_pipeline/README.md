...
study's current design (3s bullseye + 11.32s animation + 8s freeze). The
...
| **anticipatory freeze** (angry star, static, centered; target unknowable) | 8.44 s | 3.00 s |
| part2 (authority approaches target, removes star) | 11.44 s | 2.88 s |
| outcome freeze (post-punishment looking) | 14.32 s | 8.00 s |

The non-directional sound cue fires **4.92 s into part1** (7.92 s into the
recording), at the onset of the star's brief shake.

Total recording length per test trial: ~22.32 s (down from ~32.76 s under
...
- `process_icatcher_sherlock.py` crops at **14.32 s** and scores the outcome freeze.
- For the **anticipatory window (8.44–11.44 s)** run the full-video pass:
...
- `part1.gif`/`part2.gif` (both contexts, forward and reverse) are now saved
  with `loop=1` instead of `loop=0` -- with `loop=0` (infinite loop), any
  delay in the JS's src-swap timing could let the GIF restart from frame 0
  (neutral scene) right before the swap, making the shake look skipped
  entirely. `loop=1` holds on the last frame instead, which is pixel-
  identical to the anticipatory-freeze PNG it swaps to, so there's no
  visible discontinuity even under timing jitter.
