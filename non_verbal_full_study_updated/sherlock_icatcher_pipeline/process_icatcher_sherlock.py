...
still fully centered, before the 2-2.5s anticipatory pause):
  bullseye 3.0 | part1 5.44 | ANTICIPATORY FREEZE 2.5 | part2 2.88 | outcome freeze 8.0
  -> outcome freeze starts 13.82 s into the recording (the ffmpeg crop point below;
...
def analyze_gaze(icatcher_output_file, freeze_duration=8.0, anim_duration=10.82, bullseye_duration=3.0):
...
def prepare_videos(videos_dir, target_uuid=None):
    ...
    uuid), converts webm -> mp4 (cropped to start at 13.82s into the
...
def process_single_video(item, test_orders, output_dir, gpu_id, durations=None):
    ...
    # Reconstruct context if it's missing (backwards compatibility)
    if not context:
        if assigned_combo:
            if 'chains' in assigned_combo or 'distal_test_final' in assigned_combo or 'proximal_test_final' in assigned_combo:
                context = 'chains'
            elif 'single_cause' in assigned_combo or 'Test_Combo_' in assigned_combo or 'Reverse_Test_Combo_' in assigned_combo:
                context = 'single_cause'
            else:
                context = 'chains'
        else:
            context = 'chains'
    ...
    if not assigned_combo and order:
        ...
        if order == ['distal', 'proximal']:
            assigned_combo = f"{prefix}distal_test_final.gif"
        elif order == ['proximal', 'distal']:
            assigned_combo = f"{prefix}proximal_test_final.gif"
        else:
